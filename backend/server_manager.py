...
class ServerManager:
    ...
    async def start(self, config: dict) -> tuple[bool, str]:
        ...
        self._write_server_cfg(server_dir, config)
        try:
    ...
    def _write_server_cfg(self, server_dir: str, config: dict) -> None:
        """Snapshot the current config into <server_dir>/server/<identity>/cfg/server.cfg.
        CLI args still take priority at launch; this file exists so the user can
        inspect what's applied and add custom ConVars that Rust reads at startup."""
        try:
            identity = str(config.get("server_identity", "rust_server")).strip() or "rust_server"
            cfg_dir = Path(server_dir) / "server" / identity / "cfg"
            ...
            lines = [
                "# Auto-generated by Rust Server Manager on each start.",
                "# Edits here are overridden by the CLI flags passed at launch.",
                "# Add your own custom ConVars below the marker.",
                "",
                f"server.hostname {q(config.get('server_name', '') or 'Rust Server')}",
                f"server.description {q(config.get('server_description', ''))}",
                f"server.maxplayers {int(config.get('max_players', 100))}",
                f"server.saveinterval {int(config.get('save_interval', 600))}",
                f"server.identity {q(identity)}",
                f"server.port {int(config.get('server_port', 28015))}",
                f"server.queryport {int(config.get('query_port', 28017))}",
                f"server.ip {q(config.get('server_ip', '0.0.0.0'))}",
                f"server.seed {int(config.get('map_seed', 12345))}",
                f"server.worldsize {int(config.get('map_size', 3500))}",
                f"server.level {q(config.get('level', 'Procedural Map'))}",
                f"server.pve {'true' if config.get('pve') else 'false'}",
                f"server.radiation {'true' if config.get('radiation', True) else 'false'}",
                f"server.hardcore {'true' if config.get('hardcore') else 'false'}",
                f"server.gatherscale {float(config.get('gather_rate', 1.0))}",
                f"rcon.port {int(config.get('rcon_port', 28016))}",
                f"rcon.password {q(config.get('rcon_password', ''))}",
                "rcon.web 1",
                "",
                "# ── User custom ConVars below (preserved on next write) ─────────────",
            ]
            ...
            if cfg_path.exists():
                try:
                    ...
                    marker = "# ── User custom ConVars below"
                    idx = existing.find(marker)
                    if idx != -1:
                        nl = existing.find("\n", idx)
                        if nl != -1:
                            preserved = existing[nl + 1:]
                except Exception:
                    pass
